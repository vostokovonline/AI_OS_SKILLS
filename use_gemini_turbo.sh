#!/bin/bash
echo "🚀 SWITCHING BRAIN TO GEMINI 2.0 FLASH (TURBO)..."

# 1. Обновляем Граф Агентов
...
def get_model(role="DEFAULT"):
    ...
    # Используем Gemini 2.0 Flash для ВСЕХ задач.
    # Это обеспечит максимальную совместимость с инструментами.
    
    return ChatOpenAI(
        base_url=os.getenv("LLM_BASE_URL"), 
        api_key="stub", 
        model="turbo-model", # <--- Gemini 2.0 Flash Exp
        temperature=0.1, 
        request_timeout=60, 
        max_retries=2
    )
...
async def evaluator_node(state):
    ...
    llm = get_model().with_structured_output(QualityReview)
    try:
...
wf.add_node("Tools", dynamic_tool_node) # Function based
...
echo "✅ BRAIN RE-WIRED TO GEMINI TURBO."
