...
if mkdir -p "$SCHEMA_DIR"; then
  echo "✔ Created schema dir" | tee -a "$REPORT"
else
  echo "❌ Failed to create schema dir" | tee -a "$REPORT"
fi
...
if [ ! -f "$SUPERVISOR" ]; then
  echo "❌ agent_graph.py not found — skipping supervisor patch" | tee -a "$REPORT"
  exit 0
fi

cp "$SUPERVISOR" "$SUPERVISOR.bak_$TS"
echo "✔ Backup created: agent_graph.py.bak_$TS" | tee -a "$REPORT"
