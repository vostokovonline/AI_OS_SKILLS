#!/bin/bash
echo "🚀 INSTALLING AI-OS v4.0 (GROQ EDITION)..."

# 1. LiteLLM Config (Только Groq и Gemini Flash как бэкап)
...
# 2. Agent Graph (Жесткая привязка к Groq)
...
async def evaluator_node(state):
    ...
    return {"next_agent": "Supervisor"} # Skip eval for speed
...
async def supervisor_node(state):
    ...
    prompt = f"{sys}\n{usr}\n\nHISTORY:\n{str(state['messages'][-5:])}\nDECISION (Return ONLY role name: PM, CODER, RESEARCHER, FINISH):"
    try:
...
echo "✅ CONFIG UPDATED. REBUILDING..."
docker compose up -d --build --force-recreate litellm core core_worker
