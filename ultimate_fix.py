...
# Полный, исправленный код для agent_graph.py
NEW_CODE = """
import os, operator
...
# Используем MemorySaver для максимальной стабильности
...
async def supervisor_node(state):
    ...
    try:
        ...
        nxt = decision.next_node.title() # Convert RESEARCHER -> Researcher
...
# --- FIX: FUNCTION BASED TOOL EXECUTION ---
# Вместо наследования от ToolNode, мы создаем его динамически
async def run_tools_node(state):
    print("🛠 Executing Tools...")
    # 1. Объединяем статические инструменты и MCP
    current_tools = AGENT_TOOLS + mcp_manager.tools
    
    # 2. Создаем стандартный узел LangGraph
    tool_node_instance = ToolNode(current_tools)
...
# ВАЖНО: Используем нашу функцию run_tools_node вместо класса
wf.add_node("Tools", run_tools_node)
...
with open("services/core/agent_graph.py", "w", encoding="utf-8") as f:
    f.write(NEW_CODE)

print("✅ File services/core/agent_graph.py patched successfully.")
